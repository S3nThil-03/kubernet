pipeline {
    ...
    environment {
        // Define image names
        BACKEND_IMAGE = "senthil73587/docker-backend:latest"
        FRONTEND_IMAGE = "senthil73587/docker-frontend:latest"
    ...
    stages {
        stage('Checkout Code') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-senthil', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/S3nThil-03/kubernet.git", branch: 'main'
                }
            }
        }
        ...
        stage('Login to Docker Registry') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'docker-hub-senthil', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
